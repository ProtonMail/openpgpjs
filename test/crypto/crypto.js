  ...
  describe('Sign and verify', function () {
    for (let [hashName, hash] of Object.entries(openpgp.enums.hash)) {
      it(`RSA with ${hashName.toUpperCase()}`, async function () {
        ...
        const success = await crypto.signature.verify(
          openpgp.enums.publicKey.rsaEncryptSign, hash, RSAsignedData, RSAPublicParams, null, data, await crypto.computeDigest(hash, data)
        );
    ...
    for (let [hashName, hash] of Object.entries(openpgp.enums.hash)) {
      it(`DSA with ${hashName.toUpperCase()}`, async function () {
        ...
        const success = await crypto.signature.verify(
          openpgp.enums.publicKey.dsa, hash, DSAsignedData, DSAPublicParams, null, data, await crypto.computeDigest(hash, data)
        );
        expect(success).to.be.true;
      });
    }
