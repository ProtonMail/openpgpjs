...
export async function publicKeyEncrypt(algo, publicParams, privateParams, data, fingerprint) {
  switch (algo) {
    ...
    case enums.publicKey.aead: {
      ...
      const aeadMode = enums.read(enums.aead, config.preferredAEADAlgorithm);
      const { ivLength } = mode[aeadMode];
      const iv = await getRandomBytes(ivLength);
      const modeInstance = await mode[aeadMode](algoName, keyMaterial);
...
export async function publicKeyDecrypt(algo, publicKeyParams, privateKeyParams, sessionKeyParams, fingerprint) {
  switch (algo) {
    ...
    case enums.publicKey.aead: {
      ...
      const modeInstance = await mode[aeadMode.getName()](algoName, keyMaterial);
...
export function parseEncSessionKeyParams(algo, bytes) {
  ...
  switch (algo) {
    ...
    case enums.publicKey.aead: {
      ...
      const { tagLength, ivLength } = mode[aeadMode.getName()];
