...
import { getPreferredCurveHashAlgo, getHashByteLength, publicKey, isPostQuantumAlgo } from '../crypto';
...
export async function getPreferredHashAlgo(targetKeys, signingKeyPacket, date = new Date(), targetUserIDs = [], config) {
  ...
  if (isPostQuantumAlgo(signingKeyPacket.algorithm)) {
    return publicKey.postQuantum.getRequiredHashAlgo(signingKeyPacket.algorithm);
  }
